...
class PolicyConditionServiceTest {
	...
	@Test
	void shouldNotFindPolicyConditionById() {
		policyConditionList=Collections.emptyList();
		when(policyConditionRepository.findByProductId(anyString()))
		.thenReturn(policyConditionList);
		List<PolicyCondition> actual=policyConditionService.findPolicyConditionById("JIO");
		assertEquals(policyConditionList.size(),actual.size());
