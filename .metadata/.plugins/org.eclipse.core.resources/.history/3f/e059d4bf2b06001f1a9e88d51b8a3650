...
class PolicyControllerTest {
	...
	void testValidateApplicantNegativeScenario() throws Exception {
		 ...
		 PolicyResponse expectedResponse=null;
				 
				 
		when(policyRuleVerify.validatePolicy(anyString(),any(PolicyRequest.class)))
				    .thenThrow(new PolicyConditionException("PollicyConditionList not Found"));
		...
		mockMvc.perform(post("/api/v1/validate")
				...
				.andExpect(status().isOk())
				.andDo(print());
		 verify(policyEngineRequestLogService,never()).savePolicyRequestResponse(any(PolicyRequest.class),any(PolicyResponse.class));		
